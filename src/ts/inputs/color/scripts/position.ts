        ...
        if (ws.length !== 0) {
            ...
            ws.forEach(async (item, i: number): Promise<void> => {
                ...
                if (color_picker_from_palette) {
                    ...
                    const input_w = x.closest<HTMLDivElement>(
                        ws[i],
                        '.input_w',
                    );
                ...
                if (n(visualization)) {
                    ...
                    if (n(scroll_container)) {
                        ...
                        const target_fit_to_display_below_visualization: boolean = (
                            Math.ceil(
                                (fill_shadow_w_rect.top + fill_shadow_rect.height),
                            ) <= viewport_height
                        );
                        ...
                        const target_fit_to_display_to_the_right_of_visualization: boolean = (
                            Math.floor(
                                fill_shadow_w_rect.left + fill_shadow_rect.width,
                            ) <= viewport_width
                        );
                        ...
                        if (
                            ...
                            ws[i].style.top = `-${get_scroll_offset({ scroll: scroll_top }) + this.gap_between_visualization_and_color_picker + fill_shadow_rect.height}px`;
                            fill_shadows_w[i].style.top = ws[i].style.top;
                        } else {
                            ws[i].style.top = remove_double_minus({
                                val: `-${get_scroll_offset({ scroll: scroll_top }) - (this.gap_between_visualization_and_color_picker + visualization_height)}px`,
                            });
                            fill_shadows_w[i].style.top = ws[i].style.top;
                        }
                        ...
                        if (
                            ...
                            ws[i].style.left = remove_double_minus({
                                val: `-${get_scroll_offset({ scroll: scroll_left }) - (visualization_width - fill_shadow_rect.width)}px`,
                            });
                            fill_shadows_w[i].style.left = ws[i].style.left;
                        } else {
                            ws[i].style.left = `-${get_scroll_offset({ scroll: scroll_left })}px`;
                            fill_shadows_w[i].style.left = ws[i].style.left;
                        }
                    }
                }
