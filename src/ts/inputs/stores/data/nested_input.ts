...
export class NestedInput {
    ...
    public calculate_offset = ({ input }: { input: i_inputs.Input }): void => err(() => {
        ...
        if (is_child) {
            ...
            if (n(parent_el)) {
                parent_offset = x.get_numeric_css_val(
                    parent_el,
                    'margin-left',
                );
            }
    ...
    public set_parent_disbled_vals = (
        ...
        if (d_settings.Sections.i().current_section !== '') {
            ...
            Object.values(section.inputs).forEach((input_2: i_inputs.Input): void => err(() => {
                let parent_input: i_inputs.Input = input_2;
                const parents: string[] = [];

                const get_parent = (): void => err(() => {
                    if (n(parent_input)) {
                        const parent_2: i_inputs.Input = Object.values(section.inputs).find(
                            (input_3: i_inputs.Input) => err(
                        ...
                        if (n(parent_2)) {
                            ...
                            const new_parent_disabled: boolean = (
                                !parents.every((parent: string) => err(
                            ...
                            input_2.parent_disabled = (
                                input.section === d_settings.Sections.i().current_section
                                    ? new_parent_disabled
                                    : input_2.parent_disabled
                            );
                        }
    ...
    public set_all_parents_disbled_vals = (
        { sections }: { sections: {[index: string]: o_inputs.Section}; },
    ): void => err(() => {
        Object.values(sections).forEach((section: o_inputs.Section): void => err(() => {
            Object.values(section.inputs).forEach((input: i_inputs.Input): void => err(() => {
